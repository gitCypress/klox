...
internal class Scanner(private val source: String) {
    ...
    private var start = 0  // 指向正在扫描的词素中的第一个字符
    private var current = 0  // 指向当前正在考虑的字符

    fun scanTokens(): List<Token> = sequence {
        while (true) {
            start = current // 扫描新token需要重置起始标志

            val token = scanToken() ?: continue
     ...
     */
    private fun captureTokenWithVariableText(type: TokenType, literal: Any? = null): Token {
     ...
     * @return 是否符合规则
     */
    private fun peekIs(predicate: (Char) -> Boolean): Boolean = peek()?.let(predicate) ?: false
     ...
     * @return 是否符合规则
     */
    private fun peekNextIs(predicate: (Char) -> Boolean): Boolean = peekNext()?.let(predicate) ?: false

    // 是否到了tokens的末尾
    private fun isAtEnd() = current >= source.length

    // 匹配当前字符
    private fun match(expected: Char): Boolean = when (peek()) {
        expected -> {
            current++
            true
        }
